...
class InsentifhelperDetailController extends Controller
{
    ...
    public function getpakai()
    {
        ...
        if ($type_helper == '1')
        {
            $detail = PemakaianAlatDetail::where('helper1', $helper)->whereBetween('tgl_pakai',array($tgl_dr,$tgl_sp))->where('no_insentif_helper1', null)->get();
        }        
        else if ($type_helper =='2')
        {
            $detail = PemakaianAlatDetail::where('helper2', $helper)->whereBetween('tgl_pakai',array($tgl_dr,$tgl_sp))->where('no_insentif_helper2', null)->get();
        }
