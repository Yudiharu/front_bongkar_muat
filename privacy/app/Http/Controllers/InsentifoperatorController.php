...
class InsentifoperatorController extends Controller
{
    public function index()
    {
        ...
        $Operator = Operator::select('id', DB::raw("concat(nama_operator,' ~ ',nik) as namas"))->where('status_insentif', '1')->pluck('namas','id');
    ...
    public function exportPDF(){
        ...
        $insoperatordetail = InsentifoperatorDetail::where('no_insentif',$request)->orderBy('tgl_pakai')->orderBy('no_timesheet')->get();
        ...
        $setupfolder = SetupFolder::find(16);
        $tes_save = $company->kode_company.". ".$company->nama_company."/".$setupfolder->folder."/".$setupfolder->subfolder."/".$operator->nik."-".$operator->nama_operator."/".$tahun."/".$bulan."/".$request.".pdf";
    ...
    public function Post()
    {
        ...
        foreach ($insdetail as $row) {
            $cek = PemakaianAlatDetail::where('no_insentif', request()->id)->where('no_timesheet', $row->no_timesheet)->first();
            if ($cek != null) {
                $message = [
                    'success' => false,
                    'title' => 'Update',
                    'message' => 'Nomor Timesheet '.$row->no_timesheet.' sudah pernah ditarik.'
                ];
                return response()->json($message);
            }
        }
