...
class PemakaianAlatDetailController extends Controller
{
    ...
    public function cekspk()
    {
        ...
        $spk = Spk::where('no_spk', $no_spk)->first();

        if ($spk != null) {
            //Ambil Nama
            $alat = Alat::find($spk->kode_alat);
            $operator = Operator::find($spk->operator);

            if ($spk->helper1 != null){
                $helper1 = Helper::find($spk->helper1);
                $nama_helper1 = $helper1->nama_helper;
            }else {
                $nama_helper1 = null;
            }
            
            if ($spk->helper2 != null){
                $helper2 = Helper::find($spk->helper2);
                $nama_helper2 = $helper2->nama_helper;
            }else {
                $nama_helper2 = null;
            }
            
            $output = array(
                'kode_alat'=>$spk->kode_alat,
                'nama_alat'=>$alat->nama_alat,
                'operator'=>$spk->operator,
                'nama_operator'=>$operator->nama_operator,
                'helper1'=>$spk->helper1,
                'nama_helper1'=>$nama_helper1,
                'helper2'=>$spk->helper2,
                'nama_helper2'=>$nama_helper2,
            );
        }else {
            $output = array(
                'kode_alat'=>null,
    ...
    public function cekspk2()
    {
        ...
        if ($spk != null) {
            ...
            // $cekpakai = PemakaianAlatDetail::where('no_spk', $no_spk)->where('id', '<>',request()->id)->first();
            // if ($cekpakai != null){
            //     $balik = PemakaianAlatDetail::find(request()->id);
            //     $message = [
            //         'success' => false,
            //         'title' => 'Update',
            //         'message' => 'No SPK ['.$no_spk.'] ini sudah ditarik ke pemakaian alat.',
            //         'no_spk'=>$balik->no_spk
            //     ];
    ...
    public function store(Request $request)
    {
        ...
        //PROTEKSI APABILA JOB MEMILIKI SPK MAKA NOMOR SPK HARUS DIISI
        $cekjob = Spk::where('no_joborder', $head->no_joborder)->first();
        if ($cekjob != null) {
            if ($request->no_spk == null){
                $message = [
                    'success' => false,
                    'title' => 'Update',
                    'message' => 'Job ini memiliki SPK, SPK tidak boleh kosong.'
                ];
                return response()->json($message);
            }
        }
        
        //SETUP BATAS INPUT TANGGAL PAKAI ALAT
        ...
        //PROTEKSI APABILA INPUT NOMOR SPK MAKA ALAT OPERATOR HELPER TIDAK BOLEH DIUBAH.
        if ($request->no_spk != null){
            $cekspk = Spk::where('no_spk', $request->no_spk)->first();
            if ($request->kode_alat != $cekspk->kode_alat){
                $message = [
                    'success' => false,
                    'title' => 'Update',
                    'message' => 'Kode Alat berbeda dengan yang terdaftar di SPK.'
                ];
                return response()->json($message);
            }else if ($request->operator != $cekspk->operator){
                $message = [
                    'success' => false,
                    'title' => 'Update',
                    'message' => 'Kode Operator berbeda dengan yang terdaftar di SPK.'
                ];
                return response()->json($message);
            }
            // else if ($request->helper1 != $cekspk->helper1){
            //     $message = [
            //         'success' => false,
            //         'title' => 'Update',
            //         'message' => 'Kode Helper berbeda dengan yang terdaftar di SPK.'
            //     ];
            //     return response()->json($message);
            // }else if ($request->helper2 != $cekspk->helper2){
            //     $message = [
            //         'success' => false,
            //         'title' => 'Update',
            //         'message' => 'Kode Helper 2 berbeda dengan yang terdaftar di SPK.'
            //     ];
            //     return response()->json($message);
            // }
        }
        
        $tgl = $request->tgl_pakai;
        ...
        if ($tahun != $tahun_sekarang){
            $message = [
                'success' => false,
                'title' => 'Update',
                'message' => 'Error Input !!!, Tahun beda.'
            ];
            return response()->json($message);
        }
        ...
        if ($request->hitungan_pemakaian == '1') {
            ...
            if ($request->stand_by != null) {
                ...
                $hourstd = substr($request->stand_by, 0,2);
                if ($hourstd == '00') {
                    $hourstd = 24;
                }
                $standby = ($hourstd * 60) + $standmin;
            ...
            if ($tothm > 24) {
                $message = [
                    'success' => false,
                    'title' => 'Update',
                    'message' => 'Error Input !!!, Total HM lebih dari 24 Hour Meter.'
                ];
                return response()->json($message);
            }
        ...
        $simpan_tabel = [
            ...
            'jam_breakdown'=>$request->jam_breakdown,
    ...
    public function edit_detail()
    {
        ...
        $output = array(
            ...
            'jam_breakdown'=>$data->jam_breakdown,
            'jam_dr'=>$data->jam_dr,
            'jam_sp'=>$data->jam_sp,
    ...
    public function updateAjax(Request $request)
    {
        ...
        if ($request->hitungan_pemakaian == '1') {
            ...
            if ($tothm > 24){
                $message = [
                    'success' => false,
                    'title' => 'Update',
                    'message' => 'Error Input !!!, Lebih dari 24 Hour Meter.'
                ];
                return response()->json($message);
            }
        ...
        $detil = PemakaianAlatDetail::find($request->id);
        if ($detil->no_insentif != null || $detil->no_insentif_helper1 != null || $detil->no_insentif_helper2 != null){
            $message = [
                'success' => false,
                'title' => 'Update',
                'message' => 'Gagal Edit, Sudah ditarik ke Insentif.'
            ];
            return response()->json($message);
        }
