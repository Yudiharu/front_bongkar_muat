...
class InsentifoperatorDetailController extends Controller
{
    ...
    public function getpakai()
    {
        ...
        $detail = PemakaianAlatDetail::where('operator', $kode_operator)->whereBetween('tgl_pakai', array($tgldari, $tglsampai))->where('no_insentif', null)->get();
        ...
        foreach ($detail as $row) {
            ...
            if ($cekleng == null) {
                ...
                if ($row->hitungan_pemakaian == '1') {
                    ...
                    $total_insentif = floor($totaljam + $totalmin + $premi_libur);
                    ...
                    if ((float)$row->total_hm >= 100) {
                        //ratusan
                        $jamhm = substr($row->total_hm, 0,3);
                        $minhm = substr($row->total_hm, 4,2);
                    }else if ((float)$row->total_hm >= 10) {
                        //puluhan
                        $jamhm = substr($row->total_hm, 0,2);
                        $minhm = substr($row->total_hm, 3,2);
                    }else {
                        //satuan
                        $jamhm = substr($row->total_hm, 0,1);
                        $minhm = substr($row->total_hm, 2,2);
                    }
                    ...
                    $total_insentif = floor($totaljamhm + $totalminhm + $premi_libur);

                    $tothm += floor($row->total_hm);
                    $hitungmenit = substr($row->total_hm,-2);
        ...
        if ((float)$tothm >= 100) {
            ...
            // $totsmin = substr(number_format($tothm,'2'), 4,2);
        ...
        $insentifhd->total_premi = floor($totalpremijam);
        $insentifhd->total_premi_libur = floor($totalpremilibur);
        $insentifhd->gt_insentif = floor($gt_insentif);
        $insentifhd->save();
