...
class PemakaianAlatController extends Controller
{
    public function index()
    {
        ...
        $Joborder = Joborder::where('status','<>','1')->pluck('no_joborder','no_joborder');
    ...
    public function detail($joborder)
    {
        ...
        $Spk = Spk::where('no_joborder', $pemakaian->no_joborder)->pluck('no_spk','no_spk');
        ...
        return view('admin.pemakaianalatdetail.index', compact('Spk','pemakaian','pemakaiandetail','list_url','period','nama_lokasi','nama_company','no_pemakaian','Alat','Operator','Helper'));
    }
    ...
    public function Post()
    {
        ...
        $cekjo = Joborder::find($pemakaian->no_joborder);
        
        if ($cekjo->status == '1'){
            $message = [
                'success' => false,
                'title' => 'Gagal',
                'message' => 'Gagal Posting!! Pastikan Job Order sudah di Post terlebih dahulu.'
            ];
            return response()->json($message);
        }
