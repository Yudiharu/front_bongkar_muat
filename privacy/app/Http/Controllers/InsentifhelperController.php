...
class InsentifhelperController extends Controller
{
    ...
    public function exportPDF(){
        ...
        $tgl = $header->tgl_insentif;
        
        $helper = Helper::find($header->kode_helper);
        ...
        $tes_save = $company->kode_company.". ".$company->nama_company."/".$setupfolder->folder."/".$setupfolder->subfolder."/".$helper->nik."-".$helper->nama_helper."/".$tahun."/".$bulan."/".$request.".pdf";
    ...
    public function Post()
    {
        ...
        foreach ($getdetail as $row) {
            if ($getheader->type_helper == '1') {
                $cek = PemakaianAlatDetail::where('no_insentif_helper1', request()->id)->where('no_timesheet', $row->no_timesheet)->first();
            }else {
                $cek = PemakaianAlatDetail::where('no_insentif_helper2', request()->id)->where('no_timesheet', $row->no_timesheet)->first();
            }
