...
use App\Models\Invoicearpbm;
use App\Models\Invoicearumpbm;
...
class JoborderController extends Controller
{
    public function index()
    {
        ...
        $Kapal = Kapal::on('mysqlinvpbm')->where('type','<>','Tongkang')->pluck('nama_kapal','kode_kapal');
    ...
    {
        $level = auth()->user()->level;
            return Datatables::of(Joborder::with('customer1','customer2','tongkangs','kapal','vendor'))->make(true);
    }
    ...
    public function exportPDF(){
        ...
        $company_user = auth()->user()->kode_company;
        $company = Company::where('kode_company',$company_user)->first();
        
        $tahun = Carbon\Carbon::createFromFormat('Y-m-d',$tgl)->year;
        $bulan = Carbon\Carbon::createFromFormat('Y-m-d',$tgl)->month;
        
        $setupfolder = SetupFolder::find(14);
        $tes_save = $company->kode_company.". ".$company->nama_company."/".$setupfolder->folder."/".$setupfolder->subfolder."/".$tahun."/".$bulan."/".$request.".pdf";
        
        $pdf = PDF::loadView('/admin/joborder/pdf', compact('jobrequest','user','nama','nama2','nama_cetak','dt','request','date','joborder','tgl'));
    ...
    public function exportPDF2(){
        ...
        $pdf = PDF::loadView('/admin/joborder/pdf2', compact('jobrequestdetail','user','nama','nama2','dt','request', 'jobrequest','catatan_po','date', 'kode_container', 'kode_size','nojr','status_muatan','dari','tujuan','joborder','tgl','leng'));
        $pdf->setPaper([0, 0, 684, 792], 'potrait');
        return $pdf->stream('Laporan Job Request '.$request.'.pdf');
    }
    ...
    public function detail3($joborder)
    {
        $joborder = Joborder::find($joborder);
        $no_joborder = $joborder->no_joborder;
        ...
        $Alat = Alat::select('kode_alat', DB::raw("concat(nama_alat,' ~ ',no_asset_alat,' ~ ',merk) as alats"))->pluck('alats','kode_alat');
        $Operator = Operator::select('id', DB::raw("concat(nama_operator,' ~ ',nik) as namas"))->pluck('namas','id');
        $Helper = Helper::select('id', DB::raw("concat(nama_helper,' ~ ',nik) as namas"))->pluck('namas','id');
        $Mobil = Mobil::select('kode_mobil', DB::raw("concat(nopol,' ~ ',no_asset_mobil) as mobils"))->pluck('mobils','kode_mobil');
        ...
        return view('admin.spk.index', compact('joborder','list_url','period','nama_lokasi','nama_company','Mobil','Sopir','Size','Alat','Operator','Helper','Lokasi'));
    }
    ...
    public function Unpost()
    {
        ...
        $pemakaian_alat = PemakaianAlat::where('no_joborder', request()->id)->where('status','POSTED')->first();
        if ($pemakaian_alat != null){
            $message = [
                'success' => false,
                'title' => 'Update',
                'message' => 'Gagal Unpost, JO ini terhubung dengan pemakaian alat No.'.$pemakaian_alat->no_pemakaian.'.'
            ];
            return response()->json($message);
        }
        
        $trucking_non = TruckingNon::where('no_joborder', request()->id)->first();
        if ($trucking_non != null){
            $message = [
                'success' => false,
                'title' => 'Update',
                'message' => 'Gagal Unpost, JO ini terhubung dengan Trucking No.'.$trucking_non->no_truckingnon.'.'
            ];
            return response()->json($message);
        }
        
        // $arpbm = Invoicearpbm::where('no_jo', request()->id)->first();
        // if ($arpbm != null){
        //     $message = [
        //         'success' => false,
        //         'title' => 'Update',
        //         'message' => 'Gagal Unpost, JO ini terhubung dengan AR No.'.$arpbm->no_invoice.'.'
        //     ];
        //     return response()->json($message);
        // }
        
        $arumpbm = Invoicearumpbm::where('no_jo', request()->id)->first();
        if ($arumpbm != null){
            $message = [
                'success' => false,
                'title' => 'Update',
                'message' => 'Gagal Unpost, JO ini terhubung dengan AR UM No.'.$arumpbm->no_invoice.'.'
            ];
            return response()->json($message);
        }
    ...
    {
        $post = user_history::where('aksi', 'like', '%'.request()->id.'%')->where('aksi','like','Post%')->orderBy('created_at','desc')->first();
        if ($post != null) {
            $nama1 = $post->nama;
        }else {
            $nama1 = 'None';
        ...
        $unpost = user_history::where('aksi', 'like', '%'.request()->id.'%')->where('aksi','like','Unpost%')->orderBy('created_at','desc')->first();
        if ($unpost != null) {
            $nama2 = $unpost->nama;
        }else {
            $nama2 = 'None';
    ...
    public function updateAjax(Request $request)
    {
        ...
        $nama = auth()->user()->name;
        $tmp = ['nama' => $nama,'aksi' => 'Edit Job Order: '.$no_joborder.'.','created_by'=>$nama,'updated_by'=>$nama];
        user_history::create($tmp);
