...
<body onLoad="load()">
    <div class="box box-solid">
        <div class="box-body">
            <div class="box ">
                <div class="box-body">
                    <?php if (app('laratrust')->can('create-merek')) : ?>
                    <button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#addform">
                        <i class="fa fa-plus"></i> New Merek</button>
                    <?php endif; // app('laratrust')->can ?>

                    <span class="pull-right">  
                        <font style="font-size: 16px;"><b>MEREK</b></font>
                    </span>
                </div>
            </div>
    ...
    <div class="modal fade" id="editform" tabindex="-1" role="dialog">
      ...
      <button type="button" class="back2Top btn btn-warning btn-xs" id="back2Top"><i class="fa fa-arrow-up" style="color: #fff"></i> <i><?php echo e($nama_company); ?></i> <b>(<?php echo e($nama_lokasi); ?>)</b></button>
        ...
        <div id="mySidenav" class="sidenav">
            <?php if (app('laratrust')->can('update-merek')) : ?>
            <button type="button" class="btn btn-warning btn-xs edit-button" id="editmerek" data-toggle="modal" data-target="">EDIT <i class="fa fa-edit"></i></button>
            <?php endif; // app('laratrust')->can ?>

            <?php if (app('laratrust')->can('delete-merek')) : ?>
            <button type="button" class="btn btn-danger btn-xs hapus-button" id="hapusmerek" data-toggle="modal" data-target="">HAPUS <i class="fa fa-times-circle"></i></button>
            <?php endif; // app('laratrust')->can ?>
        </div>
</body>
<?php $__env->stopSection(); ?>
        ...
        $(document).ready(function() {
            ...
            $('#merek-table tbody').on( 'click', 'tr', function () {
                if ( $(this).hasClass('selected bg-gray') ) {
                    ...
                    var kode_merek = select.find('td:eq(0)').text();
                    $('.hapus-button').show();
                    $('.edit-button').show();
            ...
            $('#editmerek').click( function () {
                var select = $('.selected').closest('tr');
                var kode_merek = select.find('td:eq(0)').text();
                var row = table.row( select );
                $.ajax({
                    url: '<?php echo route('merek.edit_merek'); ?>',
                    type: 'POST',
                    data : {
                        'id': kode_merek
                    },
            ...
            $('#hapusmerek').click( function () {
                ...
                swal({
                    ...
                    if (e.value === true) {
                        $.ajax({
                            url: '<?php echo route('merek.hapus_merek'); ?>',
                            type: 'POST',
                            data : {
                                'id': kode_merek
                            },
