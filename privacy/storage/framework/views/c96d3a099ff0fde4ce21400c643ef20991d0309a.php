    ...
    <table class="grid1" style="margin-bottom: 25px; width: 100%; font-size: 9px">
        ...
        <tr style="background-color: #e6f2ff">
            ...
            <?php 
                ...
                $j = 0;
                for($i = 0; $i < $leng; $i++){ 
                    $byr3 = PembayaranDetail::with('mobil','gudangdetail')->join('pembayaran_pemilik','pembayaranpemilik_detail.no_pembayaran','=','pembayaran_pemilik.no_pembayaran')->select('pembayaran_pemilik.no_pembayaran','pembayaran_pemilik.tanggal_pembayaran','pembayaranpemilik_detail.*','pembayaran_pemilik.status','pembayaran_pemilik.kode_pemilik')->where('pembayaranpemilik_detail.no_pembayaran',$data[$i]['no_pembayaran'])->where('pembayaranpemilik_detail.no_spb',$data[$i]['no_spb'])->first();   
                    ?>
                    ...
                    <tbody>   
                            <tr class="border">
                                ...
                                <?php if ($nomor1 == null) { ?>
                                    ...
                                    <?php
                                    if ($kode_pemilik == 'SEMUA') {?>
                                        <td class="border" align="left"><?php echo $byr3->pemilik->nama_pemilik ?></td>
                                    <?php } ?>
